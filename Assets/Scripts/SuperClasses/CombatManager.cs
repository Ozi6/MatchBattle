...
{
    Direct,      // Normal projectiles that go straight to target
    Bow,         // Multiple projectiles with spread
    Bomb,        // Arc trajectory with timer explosion  
    Magic,       // Special magical effects
    Axe          // Heavy single projectile
}
...
public class CombatManager : MonoBehaviour
{
    ...
    private Dictionary<BlockType, CombatAction> combatActionDict;
    ...
    void LaunchStandardProjectiles(CombatAction action, ProjectileData projectileData, int comboSize)
    {
        ...
        // Standard projectiles: default behavior (usually 1 projectile, +1 per extra combo)
        int projectileCount = action.baseProjectileCount + Mathf.Max(0, comboSize - 3) * action.projectileCountPerCombo;
